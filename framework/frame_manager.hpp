...
public:
    ...
    Frame2(const Device& device, FencePool& fence_pool, uint32_t frame_index)
...
public:
    ...
    Prop<uint32_t, Frame2>            frame_id;

    // 后续想要使用 frame 内的 image，需要确保这个 semaphore 是 signaled
    Prop<vk::Semaphore, Frame2> submit_semaphore{VK_NULL_HANDLE};
...
{
public:
    FrameManager(const Device& device, const Swapchain& swapchain)
        : _device(device),
          _swapchain(swapchain)
    {
        ...
        {
            auto frame                      = new Frame2(_device, *_fence_pool, id);
    ...
    Frame2* acquire_frame()
    {
        ...
        if (need_recreate == EnumRecreate::NEED)
            _device.logger().warn("[need resize] acquire frame, id: {}", image_index);
    ...
    void submit_frame(Hiss::Frame2* frame)
    {
        auto need_recreate = _swapchain.submit_image(frame->frame_id.get(), frame->submit_semaphore.get());
        if (need_recreate == EnumRecreate::NEED)
            _device.logger().warn("[need resize] submit frame, id: {}", frame->frame_id.get());
    }

    // 销毁原对象，返回新对象
    static FrameManager* resize(FrameManager* old, const Device& device, const Swapchain& swapchain)
    {
        DELETE(old);
...
public:
    Prop<std::vector<Frame2*>, FrameManager> frames;
