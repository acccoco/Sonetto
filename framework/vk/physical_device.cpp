      ...
      _memory_properties(physical_device.getMemoryProperties())
{
    ...
    for (uint32_t i = 0; i < queue_properties.size(); ++i)
    {
        ...
        if (all_powerful)
            all_powerful_queue = i;
    }
    if (!all_powerful_queue.has_value())
        throw std::runtime_error("no all-powerful queue family found.");
    _queue_family_index = all_powerful_queue.value();


    // 找到合适的 depth format
    auto format = format_filter(
            {
...
std::optional<uint32_t> Hiss::GPU::graphics_queue_index() const
{
    if (_graphics_queue_family_index.empty())
        return std::nullopt;
    if (exist(_graphics_queue_family_index, EXPECT_GRAPHICS_QUEUE_INDEX))
        return EXPECT_GRAPHICS_QUEUE_INDEX;
...
std::optional<uint32_t> Hiss::GPU::present_queue_index() const
{
    if (_present_queue_family_index.empty())
        return std::nullopt;
    if (exist(_present_queue_family_index, EXPECT_PRESENT_QUEUE_INDEX))
        return EXPECT_PRESENT_QUEUE_INDEX;
...
std::optional<uint32_t> Hiss::GPU::compute_queue_index() const
{
    if (_compute_queue_family_index.empty())
        return std::nullopt;
    if (exist(_compute_queue_family_index, EXPECT_COMPUTE_QUEUE_INDEX))
        return EXPECT_COMPUTE_QUEUE_INDEX;
